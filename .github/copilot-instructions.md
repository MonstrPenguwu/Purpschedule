...
- **Default mode**, added because fully-freeform resizing was a poor fit for mobile: a box could be dragged small enough to clip its own text, or grown into overlapping a neighbour, and both required fiddly manual correction on a touchscreen. Grid mode removes that entirely — position and size are *computed*, not dragged.
- Applies to **every** canvas preset/orientation (not just portrait) — the user explicitly asked for "grid everywhere" with a toggle back to the old system, rather than grid-only-on-mobile. The freeform Drag & Drop system (above) still exists unchanged underneath; grid mode just overrides the box's `left`/`top`/`width`/`height` inline styles after the freeform ones are applied, and skips attaching `interact.js` draggable/resizable. Toggling grid mode back off re-renders straight from `state.days[k].position`/`style.width`/`style.height`, so a user's freeform arrangement is preserved untouched the whole time grid mode is on.
- **Packing algorithm** (`computeGridLayout()`): walks `DAY_KEYS` in order over just the *enabled* days. A `'full'`-span day gets its own row at full content width; consecutive `'half'`-span days pair up two-to-a-row. An unpaired trailing `'half'` (e.g. an odd number of half-span days) gets its own row, occupying the left slot with the right slot left empty — no visual placeholder, just empty canvas space.
- **Row height is fully automatic** (this was an explicit user requirement — no manual height control in grid mode, unlike width/span). For each row, every box in it is temporarily pinned to its *real* target width (a px value, not `'auto'`) with height set to `'auto'`, then measured via `getBoundingClientRect()`; the row height is the max across the row, with the same hybrid buffer as `measureNaturalBoxSize` (`v * 1.1 + 3`). Pinning to the real width first (rather than `width:'auto'`) is what makes this simpler than `measureNaturalBoxSize`: `.box-additional-times`'s `flex-wrap` measures correctly against the actual assigned width with no extra handling, unlike the freeform measurement's width-unknown case which needed a `max-width` workaround (see Drag & Drop / Resize).
- Rows stack top-to-bottom starting at `y: CANVAS_MARGIN`, separated by `GRID_ROW_GAP` (2%); `'half'` columns are separated by `GRID_GUTTER` (2%). If total content height would exceed `100 - CANVAS_MARGIN`, rows are **not** compressed or scaled down — they simply extend past the margin. Accepted the same way freeform's neighbour-overlap edge case is (see `maxSizeAvoidingNeighbors`): a real but rare edge case (very many enabled days all with long titles/extra timezones), not worth the complexity of a second shrink pass for a first version.
...
- **Default `gridSpan` is `'half'`** for every day/category (set in `mkDay()`, and reset by `applyLayoutForCategory()`/"Reset All Positions") — gives a clean 2-column grid (3 paired rows + 1 lone half for Sunday) out of the box in every orientation, landscape/square/portrait alike, since grid percentages are always relative to the canvas's own width regardless of its aspect ratio.
- The `.resize-handle` element is omitted entirely from `fillBoxContent()`'s output when `gridMode` is true (rather than just hidden via CSS) — there's nothing for it to do. `.day-box.grid-mode` gets `cursor: pointer` instead of the freeform `cursor: move`; boxes are still clickable to select/edit, just not draggable.
- UI: a global "Grid Layout" toggle lives on the BG tab (`#grid-mode-toggle`) next to Canvas Size; the Day tab's Full/Half span buttons (`#span-full-btn`/`#span-half-btn`, in `#grid-span-section`) replace the freeform Width/Height inputs (`#freeform-dims-section`) — `updateGridModeUI()` toggles which of the two sections is visible, called on init, on toggling grid mode, and from `syncAllUI()`.
...
- `gridMode` is saved in both `localStorage` and the shareable config JSON (it's a layout preference, not a secret) — loaded with an `!== undefined` check (rather than truthy) since `false` is a valid, meaningful saved value. `days[k].gridSpan` rides along with the rest of the per-day object, same as `position`/`style`.
